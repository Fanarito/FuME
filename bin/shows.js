...
let getEpisode = function (id, season_number, episode_number, fullpath, extension,done) {
    mdb.tvEpisodeInfo(
        {
            id: id,
...
let searchShow = function (filename, fullpath, extension, done) {
    ...
    shows.filter({search_name: show}).run(connection, function(err, cursor){
        cursor.toArray(function(err, result) {
            ...
            if(result.length > 0){
                // Only get episode
                getEpisode(result[0].id, season_episode[0], season_episode[1], fullpath, extension, done);
            } else {
                    ...
                    if(res.total_results != 0 && res.results[0].media_type == "tv") {
                        mdb.tvInfo({ id: res.results[0].id }, function (err, info_res) {
                            ...
                            try {
                                ...
                                getEpisode(info_res.id, season_episode[0], season_episode[1], fullpath, extension, done);
                            }
                        });
                    } else {
